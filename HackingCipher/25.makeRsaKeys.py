...
from lib import rabinMiller, cryptomath

'''
    产生RSA的key，包括公钥（n,e） 私钥（n,d）
    公钥存放于files/yourname_pubkey.txt   密钥的长度（单位：bits） n e
    私钥存放于files/yourname_prikey.txt    密钥的长度（单位：bits） n d
'''
def main():
...
#生成key并写入到文件中
#输入：name:文件名 keySize：key的size（单位：bits）
def makeKeyFiles(name, keySize):
    ...
    fo = open('files/%s_pubkey.txt' % (name), 'w')
    ...
    fo = open('files/%s_privkey.txt' % (name), 'w')
